...
app.get('/api/ransomware', async (_req, res) => {
  try {
    ...
    if (!response.ok) throw new Error(`ransomware.live returned ${response.status}`);
    ...
    console.log(`[Ransomware] Live data: ${victims.length} victims, ${groups.length} groups`);
    return res.json({ ok: true, totalVictims: victims.length, groups });
  } catch (err) {
    console.warn('[Ransomware] API failed, using fallback:', err.message);
    return res.json({
      ok: true,
      totalVictims: 6,
      fallback: true,
